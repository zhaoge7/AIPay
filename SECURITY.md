...
This threat model uses STRIDE across assets and trust boundaries, then maps concrete abuse cases to controls and verification evidence. It covers the closed-test MVP architecture through P10. Controls that require a multi-instance or production environment remain explicit residual risks and are not represented as closed by the local Gate P10.
...
| Callback reordering or duplication                      | Replay, denial         | Old failure follows success, or callback is delivered many times               | Provider event ledger, semantic digest, terminal monotonicity, idempotent ACK, Outbox, rate limiting and backlog alerts                                          | Multi-instance production must size a shared limiter and alert thresholds                                         |
...
| Fulfillment confirmation loss                           | Repudiation            | Resource is generated but provider confirmation fails                          | Persisted pending-confirm state and result, same-proof retry, success only after confirmation, backlog alert and incident workflow                               | Closed test must exercise merchant-specific recovery before onboarding                                            |
...
| Session or API Key theft                                | Spoofing               | Browser/session token or machine key is exfiltrated                            | TLS, HttpOnly/SameSite Cookie, opaque random secrets, hash-only storage, expiry, rotation, revocation, layered rate limits, aggregate alerts and browser headers | Production authentication assurance and managed secret custody remain deployment decisions                        |
...
| Sensitive data in logs or telemetry                     | Disclosure             | Error objects or provider responses include keys, identities, proof values     | Stable error catalog, no vendor messages in core, fixed timeline fields, low-sensitivity telemetry, Gitleaks and AST/schema sink audit                           | Every new sink, schema, generated bundle and telemetry exporter must enter the automated audit                    |
| Malicious dependency or build artifact                  | Tampering              | Compromised package or mutable image enters CI/deployment                      | Exact dependency versions, frozen lockfile, supply-chain age policy, pinned CI Actions and PostgreSQL image digest                                               | Production releases require SBOM, artifact scanning, signatures and provenance verification                       |
...
| Service abuse and resource exhaustion                   | Denial                 | Account, Agent, IP, or expensive endpoint is flooded                           | Bounded schemas and layered IP/credential/Agent/interface rate limits with stable 429 and Retry-After                                                            | Multi-instance deployment needs a shared limiter store and revalidated thresholds                                 |
...
- Gate P10 closes the local closed-test controls through P10-08. Production-only residual risks above remain release prerequisites and Gate P10 does not claim production readiness.
