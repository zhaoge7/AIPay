...
await run('pnpm', ['--filter', '@aipay/merchant-http-adapter', 'build']);
...
await run('pnpm', ['pack', '--out', join(outputDirectory, adapterArchive)], {
  cwd: join(repositoryRoot, 'examples/merchant-http-adapter'),
});

const archives = await Promise.all(
  [contractsArchive, sdkArchive, bridgeArchive, adapterArchive].map(async (filename) =>
